...
import { EditBook } from '@/hooks/Books';
...
const EditBookForm: React.FC<EditBookFormProps> = ({ defaultData, onClose, onSubmit }) => {
  const { data: branches } = getAllBranch();
  const { submit, loader } = EditBook();

  // Initialize form data with defaultData
  const [formData, setFormData] = useState({
    title: defaultData.title,
    author: defaultData.author,
    genre: defaultData.genre,
    quantity: defaultData.quantity.toString(), // Convert to string for controlled input
    currentBranch: defaultData.currentBranch,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let res = await submit(defaultData.id,updatedBook);
    if (!res) return;
